...
<template>
    ...
    <component :is="isAdmin ? AdminLayout : UserLayout">
        ...
        <!-- Page Header -->
        <div class="bg-white rounded-xl shadow-sm border border-pln-gray p-6 mb-6">
            <div class="flex items-center">
                <div class="w-12 h-12 bg-pln-blue rounded-xl flex items-center justify-center mr-4">
                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                    </svg>
                </div>
                <div>
                    <h1 class="text-2xl font-bold text-charcoal">📤 Upload File Baru</h1>
                    <p class="text-sm text-gray-600">Unggah dan kategorikan file ke dalam sistem SIPUS</p>
                </div>
            </div>
        </div>

        <!-- Upload Form -->
        <div class="bg-white rounded-xl shadow-sm border border-pln-gray overflow-hidden">
            <form @submit.prevent="submit" enctype="multipart/form-data">
                <div class="px-6 py-4 border-b border-pln-gray bg-pln-gray">
                    <h2 class="text-lg font-semibold text-charcoal">📋 Informasi File</h2>
                </div>

                <div class="p-6 space-y-6">
                    <!-- File Upload -->
                    <div>
                        <label for="file" class="block text-sm font-medium text-charcoal mb-2">
                            📁 File *
                        </label>
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-lg hover:border-pln-blue transition-colors duration-200">
                            <div class="space-y-1 text-center">
                                <svg v-if="!form.file" class="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                                    <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                                <div v-if="form.file" class="text-center">
                                    <p class="text-sm text-pln-blue font-medium">{{ form.file.name }}</p>
                                    <p class="text-xs text-gray-500">{{ getFileSize(form.file) }}</p>
                                </div>
                                <div class="flex text-sm text-gray-600">
                                    <label for="file" class="relative cursor-pointer bg-white rounded-md font-medium text-pln-blue hover:text-dark-blue focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-pln-blue">
                                        <span v-if="!form.file">Upload a file</span>
                                ...
                                <p class="text-xs text-gray-500">
                                    PDF, DOC, XLS, PPT, Images, TXT, ZIP up to 10MB
                                </p>
                            </div>
                        </div>
                        <div v-if="form.errors.file" class="mt-2 text-sm text-pln-red">
                            {{ form.errors.file }}
                        </div>
                    </div>
                    ...
                    <!-- Title -->
                    <div>
                        <label for="title" class="block text-sm font-medium text-charcoal mb-2">
                            🏷️ Judul File *
                        </label>
                        <input
                            id="title"
                            ...
                            required
                        />
                        <div v-if="form.errors.title" class="mt-2 text-sm text-pln-red">
                            {{ form.errors.title }}
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="rack_id" class="block text-sm font-medium text-charcoal mb-2">
                                🗂️ Rak *
                            </label>
                            <select
                                id="rack_id"
                            ...
                            <div v-if="form.errors.rack_id" class="mt-2 text-sm text-pln-red">
                                {{ form.errors.rack_id }}
                            </div>
                        </div>

                        <div>
                            <label for="sub_rack_id" class="block text-sm font-medium text-charcoal mb-2">
                                📂 Sub Rak
                            </label>
                            <select
                                id="sub_rack_id"
                            ...
                            >
                                <option value="">Pilih Sub Rak (Opsional)</option>
                                <option 
                            ...
                            <div v-if="form.errors.sub_rack_id" class="mt-2 text-sm text-pln-red">
                    ...
                    <div>
                        ...
                        ></textarea>
                        <div v-if="form.errors.description" class="mt-2 text-sm text-pln-red">
                    ...
                    <!-- Tags -->
                    <div>
                        <label for="tags" class="block text-sm font-medium text-charcoal mb-2">
                            🏷️ Tags
                        </label>
                        ...
                        <div v-if="tags && tags.length > 0" class="mt-3">
                            <p class="text-xs text-gray-500 mb-2">Tags yang sudah ada:</p>
                        ...
                        <div v-if="form.errors.tags" class="mt-2 text-sm text-pln-red">
                            {{ form.errors.tags }}
                        </div>
                    </div>
                </div>
                ...
                <!-- Form Actions -->
                <div class="px-6 py-4 bg-gray-50 border-t border-pln-gray flex items-center justify-between">
                    <a
